...
const run = async () => {
    ...
    const sectionOptions = parsedSections.map((section, i) => {
        ...
        const commentSketch = comments.map(({ line, comment }) => `${line}: ${comment}`);
        const annotationSketch = annotations.map(({ type, params }) => `${type}: ${JSON.stringify(params)}`);
        return {
            i,
    ...
    console.log();
    console.log("actions:\n");
    Object.entries(actions).forEach(([key, value]) => console.log(`${key}) ${value}`));
